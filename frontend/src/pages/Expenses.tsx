...
export default function Expenses() {
  ...
  // Independent of the table's filter/search — always the signed-in user's own month total.
  ...
  return (
    <>
      <PageHeader
        ...
        action={<Card><Metric label="Total de meus gastos no mês" value={brl(myTotal)} /></Card>}
      />
  ...
  const view = !expense.shared ? (
    ...
    <Stack gap={2}>
      ...
      <Text size="md" c="dimmed">
        {expense.participants.map((p) => members.find((m) => m.id === p)?.name ?? '—').join(', ')}
      </Text>
    </Stack>
  );
